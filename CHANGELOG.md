...
- Fixed `tessariq run --timeout` being ignored by pre- and verify-hooks: a hung `--pre` or `--verify` command no longer pins a run past `--timeout`. The run budget is now shared across pre-hooks, the agent process, and verify-hooks; a hook that would exceed the remaining budget is sent SIGTERM, given `--grace`, then SIGKILL, and the run reaches a terminal `state=timeout` with `timeout.flag` and a `phase=pre-hook`/`phase=verify-hook` tag in `runner.log`.
