...
- `promote` now rejects inconsistent task path, title, base commit, or run
  identity evidence before it creates a branch or commit. This detects partial
  edits and evidence-writing faults, not coherent tampering with the run tree.

### Fixed

- The `--mount-agent-config` warning now names the absent or unreadable host path
  that was skipped.

- Proxy mode now prints blocked destinations and the `--egress-allow` guidance
  after collecting the completed run's egress telemetry.
- Proxy teardown now times out after 30 seconds instead of letting an
  unresponsive Docker daemon hang `tessariq run`; failures remain warnings and
  do not change the run's terminal state.
