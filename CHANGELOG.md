...
- Hardened `tessariq promote` to cross-check `resolved_egress_mode` between `manifest.json` and `runtime.json`, rejecting with tamper messaging when they disagree. A proxy-mode run relabeled to `direct` in `manifest.json` can no longer suppress proxy evidence requirements.
