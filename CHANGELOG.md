...
- Reject task paths containing ASCII control bytes (`0x00`–`0x1F` or `0x7F`) at `tessariq run` so a task filename with an embedded newline can no longer smuggle forged `Signed-off-by:` or other trailers into the `tessariq promote` commit message. `ExtractTaskTitle`'s filename fallback now strips control bytes, and `tessariq promote` also refuses to build a commit message when a tampered manifest carries control characters in `task_path` or `task_title`.
...
- Hardened reconcile-time workspace cleanup so a tampered `workspace.json` can no longer redirect `tessariq attach` or `tessariq promote` into performing root-privileged `chown`/`chmod`/`os.RemoveAll` on an attacker-chosen host path. Reconcile now recomputes the canonical per-run worktree path from trusted inputs, rejects any stored `workspace_path` that does not resolve to that canonical path under symlink resolution, and `workspace.Cleanup` enforces the same `~/.tessariq/worktrees/` containment envelope as a defensive safety net.
- Restricted live worktree permissions so only the invoking host user and the runtime image's named non-root `tessariq` user can access `~/.tessariq/worktrees/<repo_id>/<run_id>/`. Tessariq now probes the runtime image for the numeric uid/gid of `tessariq`, applies owner-only mode bits to the worktree and runtime-state scratch paths, and on Linux grants the container uid exact access via POSIX ACLs plus default ACLs for future files. This preserves compatibility with custom images whose `tessariq` uid is not `1000`, avoids accidentally granting access to unrelated host users through shared gids, and keeps the parent chain at `0o700` so non-owners cannot enumerate live run IDs.
...
- Fixed `tessariq run --timeout` being ignored by pre- and verify-hooks: a hung `--pre` or `--verify` command no longer pins a run past `--timeout`. The run budget is now shared across pre-hooks, the agent process, and verify-hooks; a hook that would exceed the remaining budget is sent SIGTERM, given `--grace`, then SIGKILL, and the run reaches a terminal `state=timeout` with `timeout.flag` and a `phase=pre-hook`/`phase=verify-hook` tag in `runner.log`. A later hook in the chain can no longer start after the shared budget is already exhausted, and a deadline that fires during a fast command is now always mapped to `state=timeout` instead of racing against the wait channel.
- Fixed `tessariq attach` and `tessariq promote` failing with `workspace path is outside the canonical worktrees tree` on a previously-reconciled run whose worktree had already been removed; reconcile cleanup is now idempotent again when the canonical worktree is missing, matching the pre-TASK-090 behavior while keeping the BUG-055 containment envelope intact for genuinely-tampered `workspace.json` values.
...
- Fixed proxy teardown silently writing misleading "zero blocked destinations" evidence when Squid access-log extraction fails; teardown is now fail-closed so evidence files are only written when extraction succeeds, and `tessariq promote` rejects proxy runs with missing telemetry. Legitimate proxy runs with no blocked events (0-byte `egress.events.jsonl`) are now correctly promotable.
- Fixed `tessariq run` reporting a changed run as a successful, promotable completion when only one of `diff.patch` or `diffstat.txt` could be written: the writer now commits both artifacts atomically (tmp+rename with rollback), diff artifacts are written on every terminal state (success, failure, timeout) so changed runs remain promotable per the evidence contract, and a diff-artifact write failure on an otherwise-successful run escalates it to a non-success terminal state without masking a pre-existing failure.
- Fixed `tessariq attach` racing the runner during the brief window between `docker create` and `docker start` and reconciling a still-starting run as `state=success` with `exit_code=0`. Orphan reconciliation now treats a container in Docker's `created` state (`Running=false` with a zero-valued `FinishedAt`) as live, waiting for the supervisor to finish starting instead of fabricating a bogus terminal status and removing the container out from under the runner.
...
- Fixed orphaned-run reconciliation marking runs as `success` when the container had been reaped (by daemon prune or manual `docker rm`) before recovery; a missing container is now recorded as `failed` (or `timeout` when the timeout flag is present) with exit code `-1` so `promote` never trusts an unproven terminal state. Container cleanup is also idempotent, so a second finalization pass no longer masks the real run outcome with a "No such container" error.
