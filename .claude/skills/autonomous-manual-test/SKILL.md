...
- Sandbox mode: all file I/O must happen in `/tmp/tessariq-manual-test-<task-id>/`. Clean up after.
- Container mode: `_manual_test.go` files with `manual_test` build tag. Clean up `cmd/manual-test-*` dirs if any were created.
