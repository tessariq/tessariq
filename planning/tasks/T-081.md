...
T-079 forwarded `--model` to the OpenCode CLI, but proxy allowlist resolution still derives egress destinations exclusively from auth/config provider files in `resolveAllowlistCore`. When `--model provider/model` selects a provider that differs from the configured one, the model provider's API host is absent from the allowlist. Under the default `--egress auto`/`proxy` path with no explicit `--egress-allow`, the run fails on egress denial while `agent.json` reports `applied.model=true`.
