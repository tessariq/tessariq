...
- Blocked until `v0.2.0` becomes the active milestone; depends on the v0.1.0 closeout (`T-017`).
- Review priority #3 ("Define a clearer evidence schema ownership model, likely via `internal/evidence`").
- Should land before the v0.2.0 evidence-additions feature tasks so new artifacts adopt the shared contract from the start. Independent of `T-101` but complementary.
