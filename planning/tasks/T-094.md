...
- Idempotent cleanup (per T-085 / BUG-028) is preserved: a second `docker rm -f` attempt on the same container does not cause spurious errors.
...
- Coordinate with T-085's finalization discipline so the fix does not re-open the reconciliation path for stale `running` entries.
