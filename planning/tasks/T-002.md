...
- Follow-on tasks explicitly own filling the remaining required manifest fields before runner bootstrap begins: `task_title` in `T-003`, `base_sha` and `workspace_mode` in `T-004`, `container_name` in `T-005`, and `resolved_egress_mode` plus `allowlist_source` in `T-011`.
...
- Error-path e2e tests for invalid task-path failure are consolidated in `T-017` closeout sweep.
...
- 2026-03-29T17:30:00Z: Spec changelog renamed `--yolo` to `--interactive` (inverted default) and `--egress-allow-reset` to `--egress-no-defaults`. Code updates are tracked in T-018.
