...
- Blocked until `v0.2.0` becomes the active milestone (mirrors the milestone-gating convention used by `T-019`). Depends on the v0.1.0 closeout (`T-017`) landing first so the refactor starts from a verified-conformant baseline.
