...
- After sending cancellation to a timed-out hook, the runner honors `--grace` before escalating (matching the T-030/T-060 escalation discipline for the agent process).
...
- Normal runs where hooks complete within the budget remain unaffected; existing hook stdout/stderr capture, CWD handling (T-072), and failure propagation are preserved.
