...
- Manual test against a built CLI:
    ...
    - MT-002: replay the T-090 decoy scenario with a tampered `workspace.json` pointing at `/tmp/tessariq-manual-test-T-095/decoy` and assert the BUG-055 rejection still fires and the decoy sentinel file is untouched.
