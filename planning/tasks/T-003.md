...
- Error-path e2e tests for dirty-repo failure are consolidated in `T-017` closeout sweep.
...
- Task-path validation is intentionally owned by `T-002`; this task stays focused on dirty-repo gating and task ingestion.
