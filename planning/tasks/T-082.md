...
Both adapters return static values in `buildApplied()` independent of `cfg.Interactive`. Claude Code always returns `"interactive": true`; OpenCode (post-T-080) also returns `true`. Neither checks whether interactive was actually requested. This is capability-flag behavior.
...
- This divergence has existed since T-008 and was surfaced during T-080 code review.
