...
- Legitimate runs still clean up their worktree exactly as before, preserving the existing BUG-028/T-061 behavior (ownership repair failure does not block git worktree removal and directory deletion).
...
- Coordinate with T-089's symlink-resolution approach so the new workspace-path check uses the same `filepath.EvalSymlinks` discipline.
