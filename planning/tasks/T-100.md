...
- The zero-denied-events case remains distinguishable from telemetry extraction failure; failure paths still fail closed per T-096.
...
- Coordinate this task with T-096 so the two proxy-telemetry cases are fixed together rather than reintroducing ambiguity.
- Add this task to `T-017` dependencies so closeout cannot pass while honest zero-denied proxy runs remain unpromotable.
