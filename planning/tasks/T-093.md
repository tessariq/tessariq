...
- Cleanup on both success and failure paths still works even when the host user and container user differ in UID (BUG-028/T-061 guarantees remain intact).
...
- Add cleanup coverage that exercises both success and failure paths with the new permission scheme so T-061's ownership-repair fallback still fires when needed.
...
- Coordinate with the runtime-state layer (T-087): both paths now create per-run, non-world-readable scratch trees, and should follow consistent permission discipline.
...
- 2026-04-15T18:53:14Z: Implemented T-093 worktree hardening and runtime-state scratch hardening. Manual test pass: (evidence artifacts; path omitted) Automated checks rerun: go test ./..., go test -tags=integration ./..., go test -tags=e2e ./..., go vet ./..., go run ./cmd/tessariq --help, go run ./cmd/tessariq-workflow verify --profile spec --disposition report --json, gremlins unleash --exclude-files 'cmd/.*|internal/testutil/.*' --threshold-efficacy 70.
- 2026-04-15T19:51:26Z: Reworked T-093 after PR review: runtime image compatibility now probes the numeric identity of the named tessariq user, worktree and runtime-state hardening use owner-only mode bits plus exact-principal POSIX ACLs on Linux, Provision and runtime-state setup clean up on hardening failure, and custom images without uid 1000 now work while images missing tessariq fail fast. Manual test pass: (evidence artifacts; path omitted) Automated checks rerun: gofmt -l ., go vet ./..., go test ./..., go test -tags=integration ./..., go test -tags=e2e ./..., go run ./cmd/tessariq --help, go run ./cmd/tessariq-workflow check-skills, go run ./cmd/tessariq-workflow verify --profile task --task T-093 --disposition report --json, go run ./cmd/tessariq-workflow verify --profile spec --disposition report --json, gremlins unleash --exclude-files 'cmd/.*|internal/testutil/.*' --threshold-efficacy 70.
