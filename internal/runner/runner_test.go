...
// fakeInteractiveProcess implements ProcessRunner + interactiveCreator for
// testing the runInteractiveAttach code path.
...
func (f *fakeInteractiveProcess) Start(_ context.Context) error {
	return errors.New("Start should not be called in interactive-attach path")
}

func (f *fakeInteractiveProcess) Wait() (int, error) {
	return -1, errors.New("Wait should not be called in interactive-attach path")
}

func (f *fakeInteractiveProcess) Signal(sig os.Signal) error {
	f.mu.Lock()
...
func (f *fakeInteractiveProcess) SetOutputWriter(stdout, stderr io.Writer) {
	f.stdoutWriter = stdout
	f.stderrWriter = stderr
}

func (f *fakeInteractiveProcess) Create(_ context.Context) error {
	f.mu.Lock()
	f.createCalled = true
	f.mu.Unlock()
	return nil
...
func (f *fakeInteractiveProcess) Remove() error {
	f.mu.Lock()
	f.removeCalled = true
	f.mu.Unlock()
	return nil
...
func (f *fakeInteractiveProcess) CaptureLogs() {
	f.mu.Lock()
	f.captureCalled = true
	f.mu.Unlock()
}

func TestRunner_InteractiveAttachSuccessPath(t *testing.T) {
	t.Parallel()

	dir := t.TempDir()
	proc := &fakeInteractiveProcess{}
	...
	proc.mu.Lock()
	require.True(t, proc.createCalled, "Create must be called")
	require.True(t, proc.removeCalled, "Remove must be called")
	require.True(t, proc.captureCalled, "CaptureLogs must be called")
	proc.mu.Unlock()
}

func TestRunner_InteractiveAttachFailedProcess(t *testing.T) {
	t.Parallel()
	...
	proc.mu.Lock()
	require.True(t, proc.removeCalled, "Remove must be called even on failure")
	require.True(t, proc.captureCalled, "CaptureLogs must be called even on failure")
	proc.mu.Unlock()
}

func TestRunner_InteractiveAttachSessionReadyFiredAfterCreate(t *testing.T) {
	t.Parallel()
	...
	select {
	case <-ready:
		// OK — SessionReady fired
	default:
		t.Fatal("SessionReady must be closed in interactive-attach path")
	}

	proc.mu.Lock()
	require.True(t, proc.createCalled, "Create must be called before SessionReady")
	proc.mu.Unlock()
}

func TestRunner_InteractiveAttachContextCancelled(t *testing.T) {
	t.Parallel()
	...
	// Give the runner time to start, then cancel.
	time.Sleep(20 * time.Millisecond)
	cancel()

	// Simulate container exit after cancel.
	exitCh <- 1

	err := <-done
	var termErr *TerminalStateError
	require.ErrorAs(t, err, &termErr)
	require.Equal(t, StateKilled, termErr.State)

	proc.mu.Lock()
	require.True(t, proc.removeCalled, "Remove must be called after cancellation")
	require.True(t, proc.captureCalled, "CaptureLogs must be called after cancellation")
	proc.mu.Unlock()
}

func TestRunner_InteractiveAttachFallbackWhenNoExitCh(t *testing.T) {
	t.Parallel()

	// When InteractiveExitCh is nil, should fall back to the original
	// interactive path (Start + Wait).
	dir := t.TempDir()
	proc := newFakeProcess(0)
	r := newTestRunner(dir, proc)
	r.Config.Interactive = true
