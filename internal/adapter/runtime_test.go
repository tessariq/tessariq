...
func TestWriteRuntimeInfo_OverwritesExisting(t *testing.T) {
	...
	dir := t.TempDir()
	original := NewRuntimeInfo("ghcr.io/tessariq/original:v1", "reference", "read-only", 0, "disabled", "disabled", "open")
	require.NoError(t, WriteRuntimeInfo(dir, original))

	updated := NewRuntimeInfo("ghcr.io/tessariq/updated:v2", "custom", "read-only", 0, "enabled", "mounted", "proxy")
...
func TestRuntimeInfo_Validate(t *testing.T) {
	...
	valid := NewRuntimeInfo("test-image", "custom", "read-only", 0, "disabled", "disabled", "open")
...
func TestReadRuntimeInfo_RoundTrip(t *testing.T) {
	...
	dir := filepath.Join(t.TempDir(), "evidence")
	original := NewRuntimeInfo("test-image", "custom", "read-only", 0, "disabled", "disabled", "open")
	require.NoError(t, WriteRuntimeInfo(dir, original))
